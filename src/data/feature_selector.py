...
class FeatureSelector:
    ...
    def select_features(self, df: pd.DataFrame, verbose: bool = True) -> pd.DataFrame:
        ...
        to_drop = set()  # ✅ Usar SET para evitar duplicatas
        correlations_found = {}

        # ✅ LÓGICA SIMPLIFICADA E CORRETA
        # Estratégia: Para cada par de features correlacionadas, manter a primeira e remover as outras
        for column in upper.columns:
            # Pular se já foi marcada para remoção
            if column in to_drop:
                continue

            # Pular features essenciais (nunca remover)
            if column in self.features_to_keep:
                continue

            # Encontrar features correlacionadas com esta
            high_corr_features = upper.index[upper[column] > self.correlation_threshold].tolist()

            if not high_corr_features:
                continue

            # Verificar se alguma feature correlacionada é essencial
            essential_correlations = [f for f in high_corr_features if f in self.features_to_keep]

            if essential_correlations:
                # Se correlacionada com feature essencial, remover esta feature
                to_drop.add(column)
                correlations_found[column] = essential_correlations
            else:
                # Todas as features correlacionadas são não-essenciais
                # Manter a primeira (column), remover as outras
                for corr_feat in high_corr_features:
                    if corr_feat not in self.features_to_keep and corr_feat not in to_drop:
                        to_drop.add(corr_feat)
                        if column not in correlations_found:
                            correlations_found[column] = []
                        correlations_found[column].append(corr_feat)
